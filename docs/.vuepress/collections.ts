...
/**
 * 获取GitHub仓库根目录的所有文件夹名称（使用fetch）
 * 获取失败时返回 null
 */
import 'dotenv/config';
export async function getGitHubRepoFoldersFetch(
  owner: string = 'HEUOpenResource',
  repo: string = 'heu-icicles'
): Promise<string[] | null> {
  try {
...
const sidebarItems=await getGitHubRepoFoldersFetch();
const resourceDoc = defineCollection({
  // doc 类型，该类型带有侧边栏
  type: 'doc',
  // 文档集合所在目录，相对于 `docs`
  dir: 'resource',
